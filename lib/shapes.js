...
const { SVG_CONTAINER, HTML_WRAPPER } = require("./constants.js");
...
class Shape {
  ...
  renderSVGContainer(shapeTag) {
    return `${HTML_WRAPPER.open_tags}<svg width="${this.containerWidth}" height="${this.containerHeight}" xmlns="http://www.w3.org/2000/svg">${shapeTag}</svg>${HTML_WRAPPER.close_tags}`;
  }
